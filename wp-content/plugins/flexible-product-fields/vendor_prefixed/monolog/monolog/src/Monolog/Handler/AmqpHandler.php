...
use VendorFPF\Monolog\Formatter\JsonFormatter;
use VendorFPF\PhpAmqpLib\Message\AMQPMessage;
use VendorFPF\PhpAmqpLib\Channel\AMQPChannel;
use AMQPExchange;
/**
 * @phpstan-import-type Record from \Monolog\Logger
 */
class AmqpHandler extends \VendorFPF\Monolog\Handler\AbstractProcessingHandler
{
    /**
     ...
     */
    public function __construct($exchange, ?string $exchangeName = null, $level = \VendorFPF\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if ($exchange instanceof \VendorFPF\PhpAmqpLib\Channel\AMQPChannel) {
    ...
    private function createAmqpMessage(string $data) : \VendorFPF\PhpAmqpLib\Message\AMQPMessage
    {
        $attributes = ['delivery_mode' => 2, 'content_type' => 'application/json'];
        if ($this->extraAttributes) {
            $attributes = \array_merge($attributes, $this->extraAttributes);
        }
        return new \VendorFPF\PhpAmqpLib\Message\AMQPMessage($data, $attributes);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return new \VendorFPF\Monolog\Formatter\JsonFormatter(\VendorFPF\Monolog\Formatter\JsonFormatter::BATCH_MODE_JSON, \false);
    }
}
