 ...
 */
class BrowserConsoleHandler extends \VendorFPF\Monolog\Handler\AbstractProcessingHandler
{
    /** @var bool */
    ...
    {
        return new \VendorFPF\Monolog\Formatter\LineFormatter('[[%channel%]]{macro: autolabel} [[%level_name%]]{font-weight: bold} %message%');
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return [\VendorFPF\Monolog\Logger::DEBUG => 'debug', \VendorFPF\Monolog\Logger::INFO => 'info', \VendorFPF\Monolog\Logger::NOTICE => 'info', \VendorFPF\Monolog\Logger::WARNING => 'warn', \VendorFPF\Monolog\Logger::ERROR => 'error', \VendorFPF\Monolog\Logger::CRITICAL => 'error', \VendorFPF\Monolog\Logger::ALERT => 'error', \VendorFPF\Monolog\Logger::EMERGENCY => 'error'][$level] ?? 'log';
    }
    /**
     * @return string[]
     */
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \VendorFPF\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \VendorFPF\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
    ...
    private static function handleCustomStyles(string $style, string $string) : string
    {
        ...
        if (null === $style) {
            ...
            throw new \RuntimeException('Failed to run preg_replace_callback: ' . $pcreErrorCode . ' / ' . \VendorFPF\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $style;
    }
    /**
