...
use VendorFPF\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy;
use VendorFPF\Monolog\Handler\FingersCrossed\ActivationStrategyInterface;
 ...
 */
class FingersCrossedHandler extends \VendorFPF\Monolog\Handler\Handler implements \VendorFPF\Monolog\Handler\ProcessableHandlerInterface, \VendorFPF\Monolog\ResettableInterface, \VendorFPF\Monolog\Handler\FormattableHandlerInterface
{
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = \true, bool $stopBuffering = \true, $passthruLevel = null)
    {
        if (null === $activationStrategy) {
            $activationStrategy = new \VendorFPF\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy(\VendorFPF\Monolog\Logger::WARNING);
        }
        // convert simple int activationStrategy to an object
        if (!$activationStrategy instanceof \VendorFPF\Monolog\Handler\FingersCrossed\ActivationStrategyInterface) {
            $activationStrategy = new \VendorFPF\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy($activationStrategy);
        }
        ...
        if ($passthruLevel !== null) {
            $this->passthruLevel = \VendorFPF\Monolog\Logger::toMonologLevel($passthruLevel);
        }
        if (!$this->handler instanceof \VendorFPF\Monolog\Handler\HandlerInterface && !\is_callable($this->handler)) {
    ...
    public function reset()
    {
        ...
        if ($this->getHandler() instanceof \VendorFPF\Monolog\ResettableInterface) {
    ...
    {
        if (!$this->handler instanceof \VendorFPF\Monolog\Handler\HandlerInterface) {
