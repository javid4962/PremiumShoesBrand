...
use VendorFPF\Monolog\Formatter\LogglyFormatter;
 ...
 */
class LogglyHandler extends \VendorFPF\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $token, $level = \VendorFPF\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\extension_loaded('curl')) {
            throw new \VendorFPF\Monolog\Handler\MissingExtensionException('The curl extension is needed to use the LogglyHandler');
        }
        $this->token = $token;
    ...
    protected function send(string $data, string $endpoint) : void
    {
        ...
        \VendorFPF\Monolog\Handler\Curl\Util::execute($ch, 5, \false);
    }
    protected function getDefaultFormatter() : \VendorFPF\Monolog\Formatter\FormatterInterface
    {
        return new \VendorFPF\Monolog\Formatter\LogglyFormatter();
    }
}
