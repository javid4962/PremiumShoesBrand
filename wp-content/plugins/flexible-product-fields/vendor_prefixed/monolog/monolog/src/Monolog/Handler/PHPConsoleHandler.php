...
use VendorFPF\PhpConsole\Connector;
use VendorFPF\PhpConsole\Handler as VendorPhpConsoleHandler;
use VendorFPF\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \VendorFPF\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(array $options = [], ?\VendorFPF\PhpConsole\Connector $connector = null, $level = \VendorFPF\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\class_exists('VendorFPF\\PhpConsole\\Connector')) {
    ...
    private function initConnector(?\VendorFPF\PhpConsole\Connector $connector = null) : \VendorFPF\PhpConsole\Connector
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \VendorFPF\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \VendorFPF\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\VendorFPF\PhpConsole\Helper::isRegistered()) {
            \VendorFPF\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \VendorFPF\PhpConsole\Handler::getInstance();
    ...
    public function getConnector() : \VendorFPF\PhpConsole\Connector
    {
    ...
    {
        if ($record['level'] < \VendorFPF\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record) : void
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \VendorFPF\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \VendorFPF\Monolog\Formatter\LineFormatter('%message%');
    }
}
