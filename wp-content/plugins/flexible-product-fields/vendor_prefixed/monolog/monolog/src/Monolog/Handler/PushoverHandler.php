 ...
 */
class PushoverHandler extends \VendorFPF\Monolog\Handler\SocketHandler
{
    /** @var string */
    private $token;
     ...
     */
    public function __construct(string $token, $users, ?string $title = null, $level = \VendorFPF\Monolog\Logger::CRITICAL, bool $bubble = \true, bool $useSSL = \true, $highPriorityLevel = \VendorFPF\Monolog\Logger::CRITICAL, $emergencyLevel = \VendorFPF\Monolog\Logger::EMERGENCY, int $retry = 30, int $expire = 25200, bool $persistent = \false, float $timeout = 0.0, float $writingTimeout = 10.0, ?float $connectionTimeout = null, ?int $chunkSize = null)
    {
        ...
        $this->highPriorityLevel = \VendorFPF\Monolog\Logger::toMonologLevel($highPriorityLevel);
        $this->emergencyLevel = \VendorFPF\Monolog\Logger::toMonologLevel($emergencyLevel);
        $this->retry = $retry;
        $this->expire = $expire;
    }
    ...
    private function buildContent(array $record) : string
    {
        ...
        $message = \VendorFPF\Monolog\Utils::substr($message, 0, $maxMessageLength);
    ...
    {
        $this->highPriorityLevel = \VendorFPF\Monolog\Logger::toMonologLevel($value);
        return $this;
    }
    /**
     * @param int|string $value
    ...
    {
        $this->emergencyLevel = \VendorFPF\Monolog\Logger::toMonologLevel($value);
        return $this;
    }
    /**
