 ...
 */
class Logger implements \Psr\Log\LoggerInterface, \VendorFPF\Monolog\ResettableInterface
{
    /**
     ...
     */
    public function pushHandler(\VendorFPF\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \VendorFPF\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
     ...
     */
    public function addRecord(int $level, string $message, array $context = [], \VendorFPF\Monolog\DateTimeImmutable $datetime = null) : bool
    {
        ...
        try {
            ...
            foreach ($this->handlers as $handler) {
                if (null === $record) {
                    ...
                    $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => $datetime ?? new \VendorFPF\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
                    try {
