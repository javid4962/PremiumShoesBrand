...
abstract class AbstractPlugin extends \VendorFPF\WPDesk\PluginBuilder\Plugin\SlimPlugin
{
    ...
    public function links_filter($links)
    {
        ...
        $plugin_links = ['<a target="_blank" href="' . $support_link . '">' . \esc_html__('Support', 'flexible-product-fields') . '</a>'];
        $links = \array_merge($plugin_links, $links);
        if ($this->docs_url) {
            $plugin_links = ['<a target="_blank" href="' . $this->docs_url . '">' . \esc_html__('Docs', 'flexible-product-fields') . '</a>'];
            $links = \array_merge($plugin_links, $links);
        }
        if ($this->settings_url) {
            $plugin_links = ['<a href="' . $this->settings_url . '">' . \esc_html__('Settings', 'flexible-product-fields') . '</a>'];
            $links = \array_merge($plugin_links, $links);
        }
        return $links;
    }
