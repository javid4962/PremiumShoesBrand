...
namespace VendorFPF\WPDesk\View;
...
class PluginViewBuilder
{
    ...
    {
        $resolver = new \VendorFPF\WPDesk\View\Resolver\ChainResolver();
        $resolver->appendResolver(new \VendorFPF\WPDesk\View\Resolver\WPThemeResolver(\basename($this->plugin_dir)));
        foreach ($this->template_dirs as $dir) {
            $dir = \trailingslashit($this->plugin_dir) . \trailingslashit($dir);
            $resolver->appendResolver(new \VendorFPF\WPDesk\View\Resolver\DirResolver($dir));
        }
        return new \VendorFPF\WPDesk\View\Renderer\SimplePhpRenderer($resolver);
    }
    /**
