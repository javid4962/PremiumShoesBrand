...
if (!\class_exists('VendorFPF\\WPDesk_Tracker_Sender_Logged')) {
    class WPDesk_Tracker_Sender_Logged implements \WPDesk_Tracker_Sender
    {
        ...
        {
            if (\class_exists('VendorFPF\\WPDesk_Logger_Factory')) {
                \VendorFPF\WPDesk_Logger_Factory::log_message('Sender payload: ' . \json_encode($payload), self::LOGGER_SOURCE, \VendorFPF\WPDesk_Logger::DEBUG);
                try {
                    $response = $this->sender->send_payload($payload);
                    \VendorFPF\WPDesk_Logger_Factory::log_message('Sender response: ' . \json_encode($response), self::LOGGER_SOURCE, \VendorFPF\WPDesk_Logger::DEBUG);
                    return $response;
                } catch (\VendorFPF\WPDesk_Tracker_Sender_Exception_WpError $exception) {
                    \VendorFPF\WPDesk_Logger_Factory::log_exception($exception);
                    throw $exception;
                }
            } else {
